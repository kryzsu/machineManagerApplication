...
/**
 * REST controller for managing {@link hu.mycompany.machinemanager.domain.Holiday}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class HolidayResource {
    ...
    public ResponseEntity<Holiday> updateHoliday(@PathVariable(value = "id", required = false) final Long id, @RequestBody Holiday holiday)
        ...
        Optional<Holiday> result = holidayRepository
            ...
            .map(
                existingHoliday -> {
                    ...
                    if (holiday.getComment() != null) {
                        existingHoliday.setComment(holiday.getComment());
                    }
                    if (holiday.getDeleted() != null) {
                        existingHoliday.setDeleted(holiday.getDeleted());
                    }
