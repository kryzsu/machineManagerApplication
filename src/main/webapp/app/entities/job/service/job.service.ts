...
export class JobService {
  ...
  protected convertDateFromClient(job: IJob): IJob {
    return Object.assign({}, job, {
      createDateTime: job.createDateTime?.isValid() ? job.createDateTime.toJSON() : undefined,
      updateDateTime: job.updateDateTime?.isValid() ? job.updateDateTime.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((job: IJob) => {
        job.createDateTime = job.createDateTime ? dayjs(job.createDateTime) : undefined;
        job.updateDateTime = job.updateDateTime ? dayjs(job.updateDateTime) : undefined;
      });
    }
    return res;
  }
